    ...
    def forward(self, hidden_states: torch.Tensor, topk_output: TopKOutput):
        ...
        if use_custom_op:
            result = torch.ops.sglang.trtllm_fp4_block_scale_moe(
                ...
                hs_fp4,
                hs_scale_linear.view(torch.float8_e4m3fn).flatten(),
                self.gemm1_weights_fp4_shuffled.data,
                self.gemm1_scales_fp4_shuffled.data.view(torch.float8_e4m3fn),
                None,
                None,
                None,
                None,
                self.gemm2_weights_fp4_shuffled.data,
                self.gemm2_scales_fp4_shuffled.data.view(torch.float8_e4m3fn),
                None,
                self.g1_scale_c.data,
                self.g1_alphas.data,
                self.g2_alphas.data,
                self.num_experts,
                topk_config.top_k,
                topk_config.num_expert_group,
                topk_config.topk_group,
